...
static uint8_t mem[600 * 1024];

uint16_t requestedNextPart=0;
uint8_t *segCinematic;
uint8_t *segPalettes;
uint8_t *segBytecode;
uint8_t *_segVideo2;
uint8_t paletteIdRequested;
...
#define BYPASS_PROTECTION 1
  ...
  while (!input.quit) {
    ...
    vmcheckThreadRequests();

    sysprocessEvents(&input);
    vminp_updatePlayer(&input);
...
/*
   ...
   =======
	   ...
	   Since the virtal machine and S D L are running simultaneously in two different threads:
